...
func TestResolveFree(t *testing.T) {
	...
	for _, tt := range tests {
		...
		if len(tt.table.FreeSymbols) != len(tt.expectedFreeSymbols) {
			t.Errorf("wrong number of free symbols. got=%d, got=%+v", len(tt.table.FreeSymbols), len(tt.expectedFreeSymbols))
			continue
		}
		...
		for i, sym := range tt.expectedFreeSymbols {
			...
			if result != sym {
				t.Errorf("wong free symbol. got=%+v, want=%+v", result, sym)
			}
		}
	}
}
