...
func testConstants(input string, expected []interface{}, actual []object.Object) error {
	...
	for i, constant := range expected {
		...
		case string:
			...
			if err != nil {
				return fmt.Errorf("constatant %d - testStringObject failed: %s", i, err)
			}
		case []code.Instructions:
			fn, ok := actual[i].(*object.CompiledFunction)
			if !ok {
				return fmt.Errorf("constant %d - not a function: %T", i, fn)
			}
...
func TestCompilerScopes(t *testing.T) {
	...
	if compiler.scopeIndex != 1 {
		t.Errorf("scopeIndexWrong.  got=%d, want=%d", compiler.scopeIndex, 1)
	}
